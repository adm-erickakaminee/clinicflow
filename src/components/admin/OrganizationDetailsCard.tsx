  ...
  const loadOrganization = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('organizations')
        .select('id, name, slug, phone, email, address, cnpj, asaas_wallet_id, kyc_status, bank_account_data')
      ...
      {isReceptionist && !isAdmin ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block">Nome da Clínica</label>
              <p className="text-sm text-gray-900 font-medium">{organization?.name || 'Não informado'}</p>
            </div>
            <div>
            ...
            {organization?.slug && (
              <div>
                <label className="text-xs font-semibold text-gray-500 mb-1 block">URL de Agendamento</label>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-700 mb-2 block">URL de Agendamento (Slug)</label>
              <input
                type="text"
                value={formData.slug || ''}
          ...
          <div className="pt-4 border-t border-white/60">
            ...
            >
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Dados Bancários (KYC - Asaas)</h3>
                <p className="text-xs text-gray-500 mt-1">
            ...
            {showKYC && (
              <div className="space-y-4 bg-blue-50/50 rounded-xl p-4 border border-blue-200/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Código do Banco</label>
                    <input
                      type="text"
                      value={formData.bank_account_data?.bank_code || ''}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, bank_code: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Agência</label>
                    <input
                      type="text"
                      value={formData.bank_account_data?.agency || ''}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, agency: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                      placeholder="0000"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, account: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                      placeholder="00000"
                    />
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Dígito da Conta</label>
                    <input
                      type="text"
                      value={formData.bank_account_data?.account_digit || ''}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, account_digit: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                      placeholder="0"
                      maxLength={1}
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Tipo de Conta</label>
                    <select
                      value={formData.bank_account_data?.account_type || 'CHECKING'}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, account_type: e.target.value as 'CHECKING' | 'SAVINGS' }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                    >
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Nome do Titular</label>
                    <input
                      type="text"
                      value={formData.bank_account_data?.holder_name || ''}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, holder_name: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">CPF/CNPJ do Titular</label>
                    <input
                      type="text"
                      value={formData.bank_account_data?.holder_document || ''}
                      onChange={(e) => setFormData({
                        ...formData,
                        bank_account_data: { ...formData.bank_account_data, holder_document: e.target.value }
                      })}
                      className="w-full rounded-xl bg-white/70 border border-white/60 px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/40 transition"
                ...
                {organization?.asaas_wallet_id && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                    <p className="text-xs font-semibold text-green-900 mb-1">✅ Subconta Asaas Criada</p>
                    <p className="text-xs text-green-700 font-mono">{organization.asaas_wallet_id}</p>
                  </div>
                )}
