...
export function UnifiedCalendar({
  ...
  if (isLoading && events.length === 0 && allResources.length === 0) {
    ...
    return (
      <div
        className={`flex items-center justify-center ${theme === "light" ? "min-h-[400px]" : "h-screen"} ${bgClass}`}
      >
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col ${theme === "light" ? "bg-white" : "bg-slate-900"} ${theme === "light" ? "" : "h-screen"}`}
      >
        ...
        {/* Grid do calendário */}
        <div
          className={`${theme === "light" ? "max-h-[calc(100vh-400px)]" : "flex-1"} overflow-auto`}
        >
          {filteredResources.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <CalendarX
                  className={`w-16 h-16 mx-auto mb-4 ${theme === "light" ? "text-gray-400" : "text-slate-600"}`}
                />
                <h3
                  className={`text-lg font-medium mb-2 ${theme === "light" ? "text-gray-700" : "text-slate-300"}`}
                >
                ...
                <p
                  className={`text-xs mt-2 ${theme === "light" ? "text-gray-400" : "text-slate-500"}`}
                >
                  Debug: resources={resources.length}, additional={additionalResources.length}, all=
                  {allResources.length}, filtered={filteredResources.length}
                </p>
              </div>
            </div>
          ) : (
