...
export function TimeColumn({ 
  ...
  return (
    <div className={`flex flex-col sticky left-0 z-10 ${bgClass} border-r ${borderClass}`}>
      ...
      <div className={`h-16 border-b ${borderClass} flex items-center justify-center ${theme === 'light' ? 'bg-white' : 'bg-slate-900'}`}>
        <span className={`text-xs font-semibold ${theme === 'light' ? 'text-gray-700' : 'text-slate-500'}`}>Hora</span>
      </div>
