...
export function ResourceColumn({
  ...
  return (
    <div className={`flex flex-col min-w-[280px] border-r ${borderClass}`}>
      ...
      <div
        className={`h-16 border-b ${borderClass} flex items-center justify-center gap-3 ${headerBgClass} sticky top-0 z-10`}
      >
        {resource.avatar ? (
          <img
            src={resource.avatar}
        ...
        <div className="flex flex-col">
          ...
          <span className={`text-xs ${subtitleClass}`}>Profissional</span>
        </div>
      </div>
...
function TimeSlotDropZone({
  ...
  draggingEventId,
  theme = "dark",
}: TimeSlotDropZoneProps & { theme?: "light" | "dark" }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={clsx(`h-16 border-b ${borderClass} p-1 transition-colors duration-150`, {
