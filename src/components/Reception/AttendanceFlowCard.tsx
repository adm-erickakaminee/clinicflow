      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-xl rounded-3xl p-6">
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {activeAppointments.map((appointment) => {
            ...
            const canAddNotes = ['waiting', 'in_progress', 'medical_done'].includes(appointment.status)
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${statusBadge.className}`}>
                          {statusBadge.label}
                        </span>
                      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur flex items-center justify-center px-4">
          <div className="relative bg-white/90 backdrop-blur-xl border border-white/60 shadow-2xl rounded-2xl w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {notesAppointment.client?.full_name || 'Cliente'} • {notesAppointment.service?.name || 'Serviço'}
                </p>
              </div>
              <button
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-gray-700">Observações e Notas Médicas</label>
              <textarea
                value={notesText}
              ...
              />
              <p className="text-xs text-gray-500">Essas observações ficarão registradas no prontuário do paciente.</p>
            </div>
