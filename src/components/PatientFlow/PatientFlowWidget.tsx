...
export function PatientFlowWidget({ onRefresh }: PatientFlowWidgetProps = {}) {
  ...
  const [finishingAppointment, setFinishingAppointment] = useState<{
    id: string;
    patientName: string;
  } | null>(null);
  ...
  const organizedGroups = useMemo(() => {
    ...
    if (isReceptionist) {
      immediateAction = todayAppointments.filter(
        (apt) => normalizeStatus(apt.status || "") === "medical_done"
      );
    } else if (isProfessional) {
      immediateAction = todayAppointments.filter(
        (apt) => normalizeStatus(apt.status || "") === "in_progress"
      );
    }
    ...
    if (isReceptionist) {
      ...
      waitingQueue = todayAppointments.filter(
        (apt) => normalizeStatus(apt.status || "") === "waiting"
      );
    }
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs text-gray-600">{format(new Date(apt.start), "HH:mm")}</span>
            {duration && (
            ...
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-semibold ${statusInfo.color} ${statusInfo.border} border flex items-center gap-1`}
            >
