...
export function ServiceFlow({ appointment, organizationId, soloMode = false, gabyConfig }: ServiceFlowProps) {
  ...
  useEffect(() => {
    const loadAux = async () => {
      try {
        ...
        const histList = (historyRows || []).map((r) =>
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (evoError) {
        console.warn('Fallback para notes do appointment, tabela evolution indisponível:', evoError.message)
        await supabase
          .from('appointments')
