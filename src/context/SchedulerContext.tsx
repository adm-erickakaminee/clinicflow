...
export type SchedulerAppointment = BaseEvent & {
  ...
  status?:
    ...
    | "confirmado"
    | "pendente"
    | "cancelado"; // Mantém compatibilidade com valores antigos
  title?: string;
  patient?: string;
  procedure?: string;
...
type SchedulerContextType = {
  ...
  addAppointment: (
    data: Omit<SchedulerAppointment, "id" | "type">
  ) => Promise<{ ok: true } | { ok: false; error: string }>;
  addBlock: (
    data: Omit<SchedulerBlock, "id" | "type">
  ) => { ok: true } | { ok: false; error: string };
  addTimeOff: (
    data: Omit<SchedulerTimeOff, "id" | "type">
  ) => { ok: true } | { ok: false; error: string };
  ...
  updateAppointment: (
    id: string,
    data: Partial<Omit<SchedulerAppointment, "id" | "type">>
  ) => Promise<{ ok: true } | { ok: false; error: string }>;
...
export function SchedulerProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (currentUser?.role === "super_admin") {
        ...
        console.log(
          "🔍 Super admin com clinicId selecionado - carregando dados da clínica:",
      ...
      if (currentUser.role !== "super_admin") {
        if (!currentUser.clinicId || typeof currentUser.clinicId !== "string") {
          console.warn(
            "⚠️ load - clinicId inválido ou ausente (não é super_admin):",
            currentUser.clinicId
          );
        ...
        if (!currentUser.clinicId || typeof currentUser.clinicId !== "string") {
          console.log(
            "ℹ️ load - Super admin sem clinicId selecionado - dados não serão carregados até selecionar uma clínica"
          );
      ...
      try {
        ...
        if (!profRes.error && profRes.data) {
          ...
          const professionalsData: SchedulerProfessional[] = profRes.data
            .filter((p: Record<string, unknown>) => {
              ...
              if (currentUser?.role === "super_admin" && currentUser?.id) {
                ...
                if (
                  ...
                  console.log(
                    "🚫 SchedulerContext - Excluindo perfil do super_admin da lista de profissionais:",
                    {
                      professionalId: p.id,
            ...
            .map((p: any) => ({
              ...
              commissionModel:
                (p.commission_model as "commissioned" | "rental" | "hybrid") || "commissioned",
        ...
        if (!cliRes.error && cliRes.data) {
          ...
          const mappedClients: SchedulerClient[] = cliRes.data.map(
            (c: Record<string, unknown>) => ({
              ...
              clinicId: c.clinic_id
                ? parseInt(
                    ((c.clinic_id as string) || "1").toString().slice(0, 8).replace(/-/g, ""),
                    16
                  ) || 1
                : undefined,
        ...
        if (!appRes.error && appRes.data) {
          ...
          if (appRes.data.length > 0) {
            ...
            if (nullCount > 0) {
              ...
              console.error("❌ ========================================");
              console.error(
                "❌ Agendamentos com professional_id NULL no banco:",
                nullCount,
                "de",
                professionalIdsFromDb.length
              );
              console.error("❌ IDs dos agendamentos com professional_id NULL:");
              professionalIdsFromDb
                .filter((a: { isNull: boolean }) => a.isNull)
                .forEach((a) => {
                  console.error(
                    "   -",
                    a.appointmentId,
              ...
              console.error(
                "❌ 1. Execute o script SQL: fix_appointments_professional_id_urgent.sql"
              );
          ...
          const profileIds = [
            ...new Set(appRes.data.map((apt: any) => apt.professional_id).filter(Boolean)),
          ];
          ...
          if (!profsError && allProfessionals && allProfessionals.length > 0) {
                ...
                if (!profilesError && profiles) {
                  ...
                  profiles.forEach((profile: any) => {
                    if (
                      ...
                      professionalIdsSet.has(profile.professional_id)
                    ) {
                      profileToProfessionalMap.set(profile.id, profile.professional_id);
                      console.log(
                        "✅ Mapeado via profile.professional_id:",
                        profile.id,
                        "->",
                        profile.professional_id
                      );
                  ...
                  const stillUnmapped = profiles.filter(
                    (p: { id: string }) => !profileToProfessionalMap.has(p.id)
                  );
                  stillUnmapped.forEach((profile: { id: string; full_name?: string }) => {
                    const matchingProf = allProfessionals.find(
                      (prof: { name?: string }) =>
                        prof.name &&
                        profile.full_name &&
                  ...
                  if (finalUnmapped.length > 0 && allProfessionals.length > 0) {
                    ...
                    finalUnmapped.forEach((profile: { id: string }) => {
                      ...
                      console.warn(
                        "⚠️ Usando primeiro professional como fallback para profile_id:",
                        profile.id,
                        "->",
                        firstProfId
                      );
            ...
            if (unmappedIds.length > 0) {
              console.warn(
                "⚠️ Professional_ids dos appointments que NÃO foram mapeados:",
                unmappedIds
              );
            }
          } else if (profsError) {
          ...
          if (cliRes.data && Array.isArray(cliRes.data)) {
            cliRes.data.forEach((client: Record<string, unknown>) => {
              clientMap.set(
                client.id as string,
                ((client.full_name || client.name) as string) || "Cliente"
              );
            });
          }
          ...
          const mappedAppointments: SchedulerAppointment[] = appRes.data.map(
            (apt: Record<string, unknown>) => {
              ...
              console.log("🔍 Mapeando agendamento:", {
                ...
                hasInMap: apt.professional_id
                  ? profileToProfessionalMap.has(apt.professional_id as string)
                  : false,
              });
              ...
              if (apt.professional_id) {
                ...
                if (!professionalId && profRes.data && Array.isArray(profRes.data)) {
                  const directProf = profRes.data.find(
                    (p: { id: string }) => p.id === (apt.professional_id as string)
                  );
                  if (directProf) {
                    professionalId = directProf.id;
                    console.log(
                      "✅ SchedulerContext - professional_id já é um ID válido de professional:",
                      {
                        appointmentId: apt.id as string,
                    ...
                    console.warn(
                      "⚠️ SchedulerContext - professional_id não encontrado diretamente na lista:",
                      {
                        appointmentId: apt.id as string,
                        professional_id_from_db: apt.professional_id as string,
                        availableProfessionalIds: profRes.data
                ...
                if (!professionalId) {
                  ...
                  console.warn(
                    "⚠️ SchedulerContext - professional_id não mapeado, usando ID original:",
                    {
                      appointmentId: apt.id as string,
              ...
              const clientName = apt.client_id
                ? clientMap.get(apt.client_id as string) || "Cliente"
                : "Cliente";

              // Buscar nome do serviço
              const serviceName = apt.service_id
                ? serviceMap.get(apt.service_id as string) || undefined
                : undefined;
              ...
              return {
                ...
                clinicId:
                  parseInt(
                    ((apt.clinic_id as string) || "1").toString().slice(0, 8).replace(/-/g, ""),
                    16
                  ) || 1, // Converter UUID para número
          ...
          if (mappedAppointments.length > 0) {
            ...
            console.log(
              "📋 Todos os professionalIds mapeados:",
            ...
            if (allCount > 0) {
              ...
              console.error(
                '❌ PROBLEMA CRÍTICO: Agendamentos marcados como "all" (professional_id NULL no banco):',
              ...
              console.error(
                "❌ AÇÃO NECESSÁRIA: Execute o script SQL fix_existing_appointments_professional_id.sql para corrigir os agendamentos existentes"
              );
            }
          } else {
          ...
          if (!clinicError && clinicAppts) {
            ...
            const mappedAppointments: SchedulerAppointment[] = clinicAppts.map(
              (apt: Record<string, unknown>) => {
                const clientName = apt.client_id
                  ? clientMap.get(apt.client_id as string) || "Cliente"
                  : "Cliente";
  ...
  const checkAvailability = (
    ...
    if (professional && professional.workSchedule) {
      ...
      if (startMs < workStartMs || endMs > workEndMs) {
        console.warn(
          `⚠️ checkAvailability - Horário fora do turno (${workSchedule.start} - ${workSchedule.end})`
        );
        return false;
      }
      ...
      if (workSchedule.breakStart && workSchedule.breakEnd) {
        ...
        if (startMs < breakEndMs && endMs > breakStartMs) {
          console.warn(
            `⚠️ checkAvailability - Horário se sobrepõe ao intervalo (${workSchedule.breakStart} - ${workSchedule.breakEnd})`
          );
          return false;
        }
      }
  ...
  const mapStatusToBackend = (
    status?: string
  ): "pending" | "confirmed" | "in_progress" | "completed" | "cancelled" | "requested" => {
  ...
  const addAppointment = async (data: Omit<SchedulerAppointment, "id" | "type">) => {
    if (!canUser("create", "appointment", data.professionalId)) {
      console.error("❌ addAppointment - Acesso negado:", {
    ...
    const isUuid =
      finalProfessionalId &&
      typeof finalProfessionalId === "string" &&
      uuidRegex.test(finalProfessionalId);
    ...
    console.log("📤 addAppointment - currentUser:", {
      role: currentUser?.role,
    ...
    try {
      ...
      if (inserted) {
        ...
        const mappedAppointment: SchedulerAppointment = {
          ...
          clinicId:
            parseInt((inserted.clinic_id || "1").toString().slice(0, 8).replace(/-/g, ""), 16) || 1, // Converter UUID para número (compatibilidade)
          start: inserted.start_time,
          end: inserted.end_time,
          clientId: inserted.client_id || data.clientId,
          status: inserted.status as any,
          title:
            (data as { title?: string; procedure?: string }).title ||
            (data as { title?: string; procedure?: string }).procedure ||
            "Procedimento",
          patient:
            (data as { patient?: string; clientQuery?: string }).patient ||
            (data as { patient?: string; clientQuery?: string }).clientQuery ||
            "Paciente",
        ...
        console.log(
          "✅ addAppointment - Agendamento criado com sucesso e adicionado ao estado:",
          mappedAppointment
        );
        console.log(
          "📋 addAppointment - Total de agendamentos no estado:",
          [...appointments, mappedAppointment].length
        );
        ...
        try {
          ...
          if (!reloadError && reloadedAppts) {
            console.log(
              "🔄 addAppointment - Recarregando agendamentos do banco:",
              reloadedAppts.length
            );
            ...
            const profileIds = [
              ...new Set(
                reloadedAppts
                  .map((apt: Record<string, unknown>) => apt.professional_id as string)
                  .filter(Boolean)
              ),
            ];
            const profileToProfessionalMap = new Map<string, string>();
            ...
            if (profileIds.length > 0) {
              try {
                ...
                if (profiles) {
                  ...
                  if (professionals) {
                    profiles.forEach((profile: any) => {
                      const matchingProf = professionals.find(
                        (prof: any) =>
                          prof.name &&
                          profile.full_name &&
                          (prof.name.toLowerCase().includes(profile.full_name.toLowerCase()) ||
            ...
            const reloadedMapped: SchedulerAppointment[] = reloadedAppts.map(
              (apt: Record<string, unknown>) => {
                let professionalId = "all";

                if (apt.professional_id) {
                  professionalId =
                    profileToProfessionalMap.get(apt.professional_id as string) ||
                    (apt.professional_id as string);
                }
            ...
            console.log(
              "✅ addAppointment - Agendamentos recarregados do banco:",
              reloadedMapped.length
            );
          } else if (reloadError) {
            console.warn(
              "⚠️ addAppointment - Erro ao recarregar agendamentos:",
              reloadError.message
            );
  ...
  const addBlock = (data: Omit<SchedulerBlock, "id" | "type">) => {
    if (!canUser("block", "schedule", data.professionalId))
      return { ok: false, error: "Acesso negado" };
    ...
    supabase
      ...
      .then(({ data: inserted }) => {
        if (inserted)
          setBlocks((prev: SchedulerBlock[]) => [...prev, ...(inserted as SchedulerBlock[])]);
      });
    return { ok: true as const };
  };

  const addTimeOff = (data: Omit<SchedulerTimeOff, "id" | "type">) => {
    if (!canUser("time_off", "schedule", data.professionalId))
      return { ok: false, error: "Acesso negado" };
    const payload: any = {
      ...data,
      clinic_id: currentUser?.clinicId,
    ...
    supabase
      ...
      .then(({ data: inserted }) => {
        if (inserted)
          setTimeOffs((prev: SchedulerTimeOff[]) => [...prev, ...(inserted as SchedulerTimeOff[])]);
      });
    return { ok: true as const };
  };
  ...
  const addClient = async (client: Omit<SchedulerClient, "id"> & { id?: string }) => {
    ...
    const fullName =
      client.name || `${client.name || ""} ${client.lastName || ""}`.trim() || "Cliente";
    ...
    supabase
      ...
      .then(() => {
        setAppointments((prev) =>
          prev.map((apt) =>
            apt.id === id ? { ...apt, status: status as SchedulerAppointment["status"] } : apt
          )
        );
      });
  };
  ...
  const updateAppointment = async (
    ...
    if (!canUser("update", "appointment", data.professionalId)) {
      console.error("❌ updateAppointment - Acesso negado:", {
        role: currentUser?.role,
        professionalId: data.professionalId,
      });
      return { ok: false, error: "Acesso negado" };
    ...
    if (data.professionalId !== undefined) {
      ...
      const isUuid =
        ...
        uuidRegex.test(data.professionalId);
    ...
    if (dataWithExtras.serviceId !== undefined)
      payload.service_id = dataWithExtras.serviceId || null;
    if (dataWithExtras.notes !== undefined)
      payload.notes = dataWithExtras.notes || dataWithExtras.procedure || null;
    ...
    try {
      ...
      return {
        ok: false,
        error: err instanceof Error ? err.message : "Erro inesperado ao atualizar agendamento",
      };
    }
  };
  ...
  const updateClient = async (client: SchedulerClient) => {
    ...
    if (!canUser("update", "client", undefined, client.id)) {
      console.error("❌ updateClient - Acesso negado:", {
        role: currentUser?.role,
        clientId: client.id,
      });
  ...
  const addCashback = (clientId: string, amount: number) => {
    setClients((prev: SchedulerClient[]) =>
      prev.map((c: SchedulerClient) =>
        c.id === clientId ? { ...c, walletBalance: (c.walletBalance || 0) + amount } : c
      )
    );
  };
  ...
  const saveAnamnesis = (clientId: string, data: Anamnesis) => {
    ...
    const meta = {
      ...
      updatedBy: `Profissional ${currentUser.professionalId ?? currentUser.id}`,
    };
    setClients((prev) =>
      prev.map((c) => (c.id === clientId ? { ...c, anamnesis: { ...data, ...meta } } : c))
    );
  };
  ...
  const addEvolution = (clientId: string, evolution: Evolution) => {
    setClients((prev: SchedulerClient[]) =>
      prev.map((c: SchedulerClient) => {
        ...
        return {
          ...c,
          evolutions: [...evols, { ...evolution, id: evolution.id || String(Date.now()) }],
        };
      })
    );
  };
  ...
  const saveHealthTags = (clientId: string, tags: string[]) => {
    setClients((prev: SchedulerClient[]) =>
      prev.map((c: SchedulerClient) => (c.id === clientId ? { ...c, healthTags: tags } : c))
    );
  };
  ...
  const updateService = async (service: Service) => {
    ...
    if (!canUser("update", "service", undefined, service.id)) {
      console.error("❌ updateService - Acesso negado:", {
        role: currentUser?.role,
    ...
    setServices((prev: Service[]) =>
      prev.map((s: Service) => (s.id === service.id ? { ...s, ...service } : s))
    );
  };
  ...
  const removeService = async (id: string) => {
    ...
    if (!canUser("delete", "service", undefined, id)) {
      console.error("❌ removeService - Acesso negado:", {
        role: currentUser?.role,
        serviceId: id,
      });
  ...
  const addProfessional = async (
    p: Omit<SchedulerProfessional, "id"> & { email?: string; password?: string }
  ) => {
    if (!currentUser) {
      throw new Error("Usuário não autenticado");
    }
    ...
    if (!clinicId) {
      if (currentUser.role === "super_admin") {
        throw new Error(
          "Super admin: é necessário selecionar uma clínica ao criar o profissional."
        );
      }
      throw new Error(
        "Sem clínica definida. É necessário estar associado a uma clínica para criar profissionais."
      );
    }
    ...
    try {
      ...
      if (p.email && p.password) {
        ...
        if (authError || !authData.user) {
          ...
          throw new Error(
            `Erro ao criar conta do profissional: ${authError?.message || "Email já cadastrado ou senha inválida"}`
          );
        }
        ...
        console.log(
          "ℹ️ addProfessional - Email/senha não fornecidos, profissional será criado sem login inicial"
        );
      }
      ...
      if (professionalError || !newProfessional) {
        ...
        throw new Error(
          `Falha ao cadastrar profissional: ${professionalError?.message || "Erro desconhecido"}`
        );
      }
      ...
      if (profileUserId) {
        ...
        if (linkError) {
          ...
          console.warn(
            "⚠️ addProfessional - Profile criado mas não foi possível linkar. Pode ser atualizado manualmente."
          );
        } else {
      ...
      if ((p as any).cpf && profileUserId) {
        try {
          ...
          if (asaasError) {
            console.warn(
              "⚠️ addProfessional - Erro ao criar conta Asaas (não bloqueia cadastro):",
              asaasError
            );
        ...
        console.log(
          "ℹ️ addProfessional - CPF não fornecido ou profile não criado, conta Asaas será criada depois"
        );
      }
      ...
      const mapped: SchedulerProfessional = {
        ...
        commissionModel:
          (professionalData.commission_model as "commissioned" | "rental" | "hybrid") ||
          "commissioned",
  ...
  const updateProfessional = async (p: SchedulerProfessional) => {
    ...
    if (
      (p as any).payout_model ||
      (p as any).payout_percentage !== undefined ||
      (p as any).fixed_monthly_payout_cents !== undefined ||
      (p as any).cpf !== undefined ||
      (p as any).bank_account_data !== undefined
    ) {
    ...
    setProfessionals((prev: SchedulerProfessional[]) =>
      prev.map((prof: SchedulerProfessional) => (prof.id === p.id ? { ...prof, ...p } : prof))
    );
  };
  ...
  const removeProfessional = async (id: string) => {
    ...
    if (!canUser("delete", "professional", undefined, id)) {
      console.error("❌ removeProfessional - Acesso negado:", {
        role: currentUser?.role,
        professionalId: id,
      });
    ...
    if (error) throw error;
    setProfessionals((prev: SchedulerProfessional[]) =>
      prev.filter((p: SchedulerProfessional) => p.id !== id)
    );
  };
  ...
  const updateClinic = async (data: { name?: string; phone?: string }) => {
    ...
    if (result.error) {
      console.warn(
        "⚠️ Erro ao atualizar em organizations, tentando clinics:",
  ...
  const provisionNewClinic = async (
    ...
    if (currentUser?.role !== "super_admin") {
      console.error(
        "❌ provisionNewClinic - Acesso negado: apenas super_admin pode provisionar clínicas"
      );
    ...
    try {
      ...
      if (adminData.useExisting && adminData.existingAdminId) {
        ...
        if (profileError || !existingProfile) {
          console.error(
            "❌ provisionNewClinic - Profile do administrador não encontrado:",
            profileError
          );
          return {
            ok: false,
            error: "Administrador selecionado não encontrado. Por favor, selecione outro.",
          };
        }
      } else {
        // Criar novo administrador via auth.signUp
        console.log("🔄 provisionNewClinic - Criando novo administrador...", {
        ...
        if (authError || !authData.user) {
          ...
          if (authError?.status === 429 || authError?.message?.includes("48 seconds")) {
            return {
              ok: false,
              error:
                "Muitas tentativas de cadastro. Por favor, aguarde 1 minuto antes de tentar novamente. (Rate limit: 48 segundos)",
            };
          }
          ...
          if (
            ...
            return {
              ok: false,
              error:
                'Este email já está cadastrado. Se você já tem uma conta, use a opção "Usar administrador existente".',
            };
          }
          ...
          return {
            ...
            error:
              authError?.message ||
              "Erro ao criar conta do administrador. Verifique se o email já não está cadastrado.",
          };
        }
...
3. Se o role não for 'super_admin', atualize:
   UPDATE profiles SET role = 'super_admin' WHERE id = '${authUser?.id}';`;
        ...
        if (createdAuthUser) {
          console.warn(
            "⚠️ provisionNewClinic - Organização não criada, mas usuário foi criado. Tentando limpar..."
          );
          ...
          console.warn(
            "⚠️ Usuário criado que precisa ser removido manualmente:",
            createdAuthUser.id
          );
        }
...
3. Se necessário, atualize role: UPDATE profiles SET role = 'super_admin' WHERE id = '${authUser?.id}';`;
      ...
      if (!adminData.useExisting) {
        ...
        if (profileCreated) {
          ...
          if (verifyError) {
            ...
            profileError = {
              ...
              message: "Profile não foi encontrado após tentativa de criação",
            };
          } else {
        ...
        if (!profileCreated || profileError) {
          ...
          let detailedError =
            profileError?.message || "Erro ao criar/atualizar perfil do administrador.";
...
Isso permite que super_admin crie profiles para outros usuários.`;
        ...
        console.log(
          "✅ provisionNewClinic - Profile do administrador criado/atualizado com sucesso"
        );
      } else {
        // Se está usando admin existente, atualizar o profile para vincular à nova clínica
        console.log(
          "🔄 provisionNewClinic - Atualizando profile existente para vincular à nova clínica..."
        );
...
para permitir que super_admin atualize profiles de outros usuários.`;
      ...
      return {
        ok: false,
        error: error.message || "Erro inesperado ao provisionar clínica. Tente novamente.",
      };
    }
  };
    ...
    try {
      ...
      if (existingAdmin) {
        return {
          ok: false,
          error:
            "Esta clínica já possui um administrador. Desvincule o administrador atual antes de adicionar um novo.",
        };
      }
      ...
      if (authError || !authData.user) {
        ...
        if (authError?.status === 429 || authError?.message?.includes("48 seconds")) {
          return {
            ok: false,
            error:
              "Muitas tentativas de cadastro. Por favor, aguarde 1 minuto antes de tentar novamente.",
          };
        }
      ...
      if (profileError) {
        ...
        console.error(
          "❌ addAdminToClinic - Falha ao " + operation + " profile. ID do usuário:",
          adminUserId
        );
        ...
        if (profileError.code === "42501") {
          errorMessage =
            `Erro de permissão RLS ao ${operation} profile. Verifique se você tem permissão para criar/atualizar administradores.\n\n` +
            `Código: ${profileError.code}\n` +
            `Mensagem: ${profileError.message}\n` +
            `Dica: ${profileError.hint || "Execute o script rls_complete_access_control.sql no Supabase"}`;
        } else if (profileError.code === "23505") {
          errorMessage =
            "Já existe um perfil para este usuário. Por favor, tente novamente ou entre em contato com o suporte.";
        } else if (profileError.code === "23503") {
  ...
  const deleteClinic = async (
    ...
    try {
      console.log(
        "🔄 deleteClinic - Processando clínica:",
        clinicId,
        deactivateOnly ? "(desativar)" : "(excluir)"
      );
          ...
          return {
            ...
            error:
              deleteError.code === "42501"
                ...
                : deleteError.message ||
                  "Erro ao excluir clínica. Pode haver dados vinculados que impedem a exclusão.",
          };
        }
  ...
  useEffect(() => {
    ...
    const loadUserProfile = async (authUser: any) => {
      console.log("🔄 loadUserProfile - Iniciando...", {
        userId: authUser?.id,
        email: authUser?.email,
      });
      ...
      try {
              ...
              setTimeout(
                () =>
                  resolve({
                    data: null,
                    error: {
                      message: "Query timeout após 3 segundos - possível problema de RLS",
                      code: "TIMEOUT",
                    },
                  }),
                3000
        ...
        if (
          errorWithFlag &&
          (errorWithFlag.message?.includes("is_super_admin") || errorWithFlag.code === "PGRST116")
        ) {
          console.log(
            "🔄 loadUserProfile - Coluna is_super_admin não existe ou erro, buscando sem ela..."
          );
        ...
        if (profileError) {
          ...
          try {
            ...
            if (storedUser) {
              ...
              if (parsed.id === authUser.id && parsed.role === "super_admin") {
                ...
                console.log(
                  "✅ loadUserProfile - Preservando dados do super_admin do localStorage:",
                  preservedUser
                );
          ...
          console.warn(
            "⚠️ Erro ao buscar perfil - não definindo role padrão. Aguardando confirmação do banco."
          );
          setCurrentUser(null);
        ...
        if (!profile) {
          ...
          try {
            ...
            if (storedUser) {
              ...
              if (parsed.id === authUser.id && parsed.role === "super_admin") {
                ...
                console.log(
                  "✅ loadUserProfile - Preservando dados do super_admin do localStorage (sem perfil no banco):",
                  preservedUser
                );
                setCurrentUser(preservedUser);
                localStorage.setItem("clinicflow_user", JSON.stringify(preservedUser));
          ...
          // Retornar null e deixar sessionLoading = true até que o role seja confirmado
          console.warn(
            "⚠️ Perfil não encontrado - não definindo role padrão. Aguardando confirmação do banco."
          );
          setCurrentUser(null);
          localStorage.removeItem("clinicflow_user"); // Limpar localStorage incorreto
          return null;
        ...
        try {
          ...
          if (storedUser) {
            ...
            if (parsed.role === "super_admin" && parsed.id === authUser.id) {
              ...
              console.log(
                "🔍 loadUserProfile - Preservando role e dados do super_admin do localStorage:",
                preservedData
              );
            }
          }
        ...
        const isSuperAdmin =
          isSuperAdminFlag || roleFromDb === "super_admin" || preservedRole === "super_admin";
        ...
        if (isSuperAdmin || preservedRole === "super_admin") {
          ...
          roleFromDb &&
          ["admin", "clinic_owner", "receptionist", "professional", "client"].includes(roleFromDb)
        ) {
          ...
          preservedRole &&
          ["admin", "clinic_owner", "receptionist", "professional", "client"].includes(
            preservedRole
          )
        ) {
          ...
          try {
            ...
            if (storedUser) {
              ...
              if (parsed.clinicId && parsed.role === "super_admin") {
                ...
                console.log(
                  "🔍 Preservando clinicId do localStorage para super_admin:",
                  clinicIdForSuperAdmin
                );
              }
            }
          } catch (e) {
        ...
        const clinicId = isSuperAdmin
          ? clinicIdForSuperAdmin || preservedData?.clinicId || profile.clinic_id || null
        ...
        const avatarUrl = profile.avatar_url || (isSuperAdmin && preservedData?.avatarUrl) || "";
      ...
      if (event === "SIGNED_OUT") {
        ...
        if (session?.user) {
          ...
          try {
            ...
            if (!loadedUser) {
              console.error(
                "❌ onAuthStateChange - Não foi possível carregar perfil do banco de dados!"
              );
              setCurrentUser(null);
              hasLoadedUserRef.current = false;
              localStorage.removeItem("clinicflow_user");
        ...
        if (hasLoadedUserRef.current || currentUserRef.current || isInitializingRef.current) {
          console.log(
            "ℹ️ onAuthStateChange - Usuário já carregado ou inicialização em andamento, ignorando INITIAL_SESSION"
          );
          return;
        }
        ...
        if (session?.user) {
          console.log(
            "✅ INITIAL_SESSION detectada, mas initializeSession já deve ter carregado. Verificando..."
          );
          ...
          setTimeout(() => {
            if (
              ...
              console.log(
                "⚠️ INITIAL_SESSION - initializeSession não carregou, carregando agora..."
              );
              isInitializingRef.current = true;
              setSessionLoading(true);
