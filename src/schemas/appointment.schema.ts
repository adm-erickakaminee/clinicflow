...
export function validateAppointmentMove(
...
): { success: true; data: z.infer<typeof updateAppointmentSchema> } | { success: false; error: string } {
  ...
  if (data.serviceId) {
    ...
    if (!isQualified) {
      return {
        success: false,
        error: '⚠️ Este profissional não está habilitado para realizar este serviço. Mova para outro profissional qualificado.',
      }
    }
  }
