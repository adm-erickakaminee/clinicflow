...
export function AdminAnalyticsView() {
  ...
  const loadFinancialMetrics = async (startISO: string, endISO: string) => {
    ...
    if (metrics.monthlyGoalCents > 0 && period !== "month") {
      ...
      metrics.monthlyGoalCents = Math.round(
        (metrics.monthlyGoalCents * daysInPeriod) / daysInMonth
      );
    }
  ...
  const loadClientMetrics = async (startISO: string, endISO: string) => {
    ...
    const { data: clientData, error: clientError } = await supabase
      .from("financial_transactions")
      .select(
        "appointment_id, amount_cents, appointment:appointments(client_id, client:clients(full_name))"
      )
      .eq("clinic_id", clinicId)
      .eq("status", "completed")
      .gte("created_at", startISO)
  ...
  const loadRevenueEvolution = async (
    ...
    try {
      ...
      const currentRevenue = currentData?.reduce((sum, tx) => sum + (tx.amount_cents || 0), 0) || 0;
    ...
    <div className="space-y-6">
      ...
      {dateRange && (
        <div className="rounded-3xl bg-white/60 border border-white/40 shadow-xl p-6">
          ...
          period === "semester" ||
          period === "year" ? (
            <PeriodAppointmentsChart appointments={periodAppointments} dateRange={dateRange} />
          ) : (
          ...
          {appointmentRisks.length > 0 ? (
            <div className="space-y-2">
              {appointmentRisks.map((risk) => (
                <div key={risk.id} className="rounded-xl bg-red-50 border border-red-200 p-3">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{slot.idle_hours.toFixed(1)}h ociosas</p>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        ...
                        {prof.goalProgress !== undefined && (
                          <span className="ml-2">• Meta: {prof.goalProgress.toFixed(0)}%</span>
                        )}
                      </p>
                    </div>
                  </div>
        ...
        {/* Profissional do Mês */}
        <div className="rounded-3xl bg-white/60 border border-white/40 shadow-xl p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">⭐ Profissional Destaque</h3>
          {topProfessionals.length > 0 ? (
            <div className="space-y-4">
              {topProfessionals
                ...
                <p className="text-sm text-gray-500">Nenhum profissional com meta configurada</p>
              )}
            </div>
          ) : (
            <p className="text-sm text-gray-500">Nenhum profissional encontrado no período</p>
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <div className="relative overflow-x-auto pb-2" style={{ scrollbarWidth: "thin" }}>
          <div
            className="relative flex items-end justify-start h-72 gap-1.5 z-10"
            ...
            {displayData.map((day, index) => {
              ...
              return (
                ...
                >
                  ...
                  {hasAppointments && (
                    <div
                      className={`absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-semibold z-20 ${
                  ...
                  <div className="mt-2 text-center w-full">
                    ...
                    <div
                      className={`text-[10px] mt-0.5 ${isActive ? "text-orange-500 font-semibold" : "text-gray-400"}`}
                    >
