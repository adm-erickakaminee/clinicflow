...
const statusStyles: Record<string, { badge: string; text: string; muted?: boolean }> = {
  confirmed: {
...
export function AppointmentsListView() {
  ...
  const filtered = useMemo(() => {
    ...
    return appointments
      ...
      .filter(
        (apt) => selectedProfessional === "all" || apt.professionalId === selectedProfessional
      )
  ...
  const groupedByPro = useMemo(() => {
    ...
    professionals.filter((p) => p.id !== "all").forEach((p) => map.set(p.id, []));
  ...
  const handleNewAppointment = (profId: string) => {
    const fallbackPro =
      profId === "all" ? (professionals.find((p) => p.id !== "all")?.id ?? "") : profId;
    setModalData({
      mode: "create",
      profId: fallbackPro,
      time: "09:00",
  ...
  const renderStatus = (status?: string) => {
    ...
    return (
      <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${badge}`}>{text}</span>
    );
  };
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-xl rounded-3xl p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-sm font-semibold border ${
                dateFilter === "today"
                  ...
                  : "bg-white/70 text-gray-800 border-white/70"
              }`}
            >
              Hoje
            </button>
            ...
            <p className="text-sm text-gray-500 ml-2">{format(targetDate, "dd 'de' MMMM")}</p>
          </div>
        ...
        {isAll ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      {profEvents.length === 0 && (
                        <p className="text-xs text-gray-500">Sem eventos nesta data.</p>
                      )}
                    </div>
                  </div>
                );
          ...
          <div className="space-y-3">
            {filtered.map((ev) => {
              const isCancelled =
                ev.status === "cancelado" || (ev.status as string) === "cancelled";
              return (
                <div
                  key={ev.id}
                ...
                >
                  <div>
                    <p className="text-sm font-semibold text-gray-900">
                      {ev.title ?? ev.patient ?? "Agendamento"}
                    </p>
                    <p className="text-xs text-gray-500">
            ...
            {filtered.length === 0 && (
              <p className="text-sm text-gray-600">Sem agendamentos nesta data.</p>
            )}
          </div>
        )}
