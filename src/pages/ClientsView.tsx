...
export function ClientsView() {
  ...
  const clientAppointments = useMemo(
    () => appointments.filter((a) => a.clientId === selected?.id).sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime()),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[360px,1fr] gap-6">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-900">{c.name || 'Cliente sem nome'}</p>
                  <p className="text-xs text-gray-500">{c.mobile || c.phone || 'Sem telefone'}</p>
                </div>
                {hasBalance ? <Wallet2 className="h-4 w-4 text-amber-500" /> : <AlertTriangle className="h-4 w-4 text-gray-300" />}
              </button>
            )
          })
          )}
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-xl rounded-3xl p-5 space-y-4">
        {!selected ? (
          <div className="h-full flex items-center justify-center text-gray-600 text-sm">Selecione um cliente para visualizar.</div>
        ) : (
          <>
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-semibold text-gray-900">{selected.name || 'Cliente sem nome'}</p>
                  <p className="text-xs text-gray-500">{selected.email ?? selected.mobile ?? selected.phone ?? 'Sem contato'}</p>
                </div>
              </div>
              <button
              ...
              <div className="rounded-2xl bg-gradient-to-r from-amber-500 via-amber-600 to-black text-white px-4 py-3 shadow-lg w-full lg:w-80">
                ...
                <p className="text-2xl font-semibold">R$ {selected.walletBalance?.toFixed(2) ?? '0,00'}</p>
                {isAdminView && <button className="mt-2 text-xs font-semibold text-white/90 underline underline-offset-4">Ajustar manualmente</button>}
              </div>
            </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-sm font-semibold border transition ${
                    tab === t.key ? 'bg-gray-900 text-white border-gray-900' : 'bg-white/70 text-gray-800 border-white/70 hover:bg-white'
                  }`}
                >
                  {t.label}
                </button>
            ...
            {tab === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {isAdminView ? (
                  <MetricCard icon={<Activity className="h-4 w-4 text-emerald-600" />} label="LTV" value={`R$ ${ltv.toFixed(2)}`} />
                ) : (
                  <MetricCard icon={<Activity className="h-4 w-4 text-emerald-600" />} label="Nível de Fidelidade" value={loyaltyLevel} />
                )}
                <MetricCard icon={<Stethoscope className="h-4 w-4 text-blue-600" />} label="Frequência" value={`${freq} visitas`} />
                <MetricCard
                ...
                {!isAdminView && (
                  <>
                    <Card title="Tags de Comportamento">
                      <TagList items={['Pontual', 'Prefere WhatsApp']} empty="Sem tags." color="blue" />
                    </Card>
                    <Card title="Preferências">
                      <p className="text-sm text-gray-700">Prefere contato por Whats. Café sem açúcar.</p>
                    </Card>
                    <Card title="Pendências">
                      <div className="space-y-1 text-sm text-gray-800">
                        <label className="flex items-center gap-2">
                          <input type="checkbox" className="rounded border-gray-300" /> Termo Assinado?
                        </label>
                        <label className="flex items-center gap-2">
                ...
                {clientAppointments.map((a) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{a.procedure ?? a.title ?? 'Procedimento'}</p>
                    </div>
                    <StatusBadge status={a.status} />
                  </div>
                ))}
                {clientAppointments.length === 0 && <p className="text-sm text-gray-600">Nenhum agendamento encontrado.</p>}
              </div>
            )}
          </>
        )}
      ...
      <AppointmentModal
        ...
        professionals={professionals.filter((p) => p.id !== 'all').map((p) => ({
          id: p.id,
          name: p.name,
...
function TagList({ items, empty, color = 'amber' }: { items: string[]; empty: string; color?: 'amber' | 'blue' }) {
  ...
  const colorMap = color === 'amber' ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-blue-100 text-blue-800 border-blue-200'
  return (
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur flex items-center justify-center px-4">
      <div className="relative bg-white/90 backdrop-blur-xl border border-white/60 shadow-2xl rounded-2xl w-full max-w-lg p-6 space-y-4">
        ...
        <div className="space-y-3">
          <Input label="Nome" value={form.name} onChange={(v) => setForm((p) => ({ ...p, name: v }))} />
          <Input label="Email" value={form.email} onChange={(v) => setForm((p) => ({ ...p, email: v }))} />
          <Input label="WhatsApp" value={form.mobile} onChange={(v) => setForm((p) => ({ ...p, mobile: v }))} />
          <div className="space-y-1">
