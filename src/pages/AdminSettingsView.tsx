...
export function AdminSettingsView() {
  ...
  const [professionalsKYC, setProfessionalsKYC] = useState<
    Array<{
      id: string;
      full_name: string | null;
      cpf: string | null;
  ...
  const loadOrganization = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("organizations")
        .select(
          "platform_fee_override_percent, asaas_wallet_id, kyc_status, status, subscription_renewal_date, subscription_cancelled_at, included_users_count"
        )
        .eq("id", clinicId)
        .maybeSingle();
      ...
      const { data: usersData, error: usersError } = await supabase.rpc("count_active_users", {
  ...
  const handleRequestAsaasSubaccount = async (type: "clinic" | "professional", id?: string) => {
    ...
    try {
      ...
      if (type === "clinic") {
        ...
        toast.success(
          "Subconta Asaas criada com sucesso! Status: " + (data.status || "Em análise")
        );
      } else if (id) {
        ...
        if (!profile?.cpf || !profile?.bank_account_data) {
          toast.error(
            "Dados KYC incompletos. Preencha CPF e dados bancários no cadastro do profissional."
          );
          return;
        }
  ...
  const handleRequestClientPasswordReset = async () => {
    ...
    try {
      ...
      toast.success(
        `Solicitação de redefinição de senha enviada para ${clientSearchResult.full_name}`
      );
        ...
        <div className="rounded-3xl bg-white/60 border border-white/40 shadow-xl p-6">
          ...
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">
              Forçar Reset de Senha (Operacional)
            </h4>
                    ...
                    <button
                      onClick={() =>
                        handleForcePasswordReset(
                          receptionist.id,
                          receptionist.full_name || "Usuário"
                        )
                      }
          ...
          <div className="border-t border-white/40 pt-6">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">
              Acesso à Conta do Cliente (Suporte)
            </h4>
              ...
              {clientSearchResult && (
                <div className="rounded-xl bg-blue-50 border border-blue-200 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">
                        {clientSearchResult.full_name}
                      </p>
                      <p className="text-xs text-gray-600">
                        {clientSearchResult.email || "Sem email"}
                      </p>
                    </div>
                    <button
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-600">
                      ...
                      {prof.asaas_wallet_id && (
                        <span className="font-mono">
                          Wallet: {prof.asaas_wallet_id.substring(0, 8)}...
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <div className="rounded-3xl bg-white/60 border border-white/40 shadow-xl p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900">
              Perfil da Clínica - Turnos e Custos
            </h3>
          </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-semibold text-gray-900">
                      {feeLedgerBalance.transaction_count}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="border-t border-gray-200 pt-3 mt-3">
                ...
                {activeUsersCount > (organization.included_users_count || 2) && (
                  <p className="text-xs text-amber-600 mt-1">
                    Você tem {activeUsersCount - (organization.included_users_count || 2)}{" "}
                    usuário(s) extra(s)
                  </p>
                )}
              </div>
            ...
            {organization.status === "cancelled" && organization.subscription_cancelled_at && (
              <div className="rounded-xl bg-gray-50 border border-gray-200 p-4">
                <p className="text-sm text-gray-600">
                  ...
                  {new Date(organization.subscription_cancelled_at).toLocaleDateString("pt-BR")}
                </p>
              </div>
            )}
          </div>
            ...
            {settings?.monthly_revenue_goal_cents && settings.monthly_revenue_goal_cents > 0 && (
              <p className="mt-2 text-sm text-gray-600">
                ...
                <span className="font-semibold">
                  {formatCurrency(settings.monthly_revenue_goal_cents)}
                </span>
              </p>
            )}
          </div>
