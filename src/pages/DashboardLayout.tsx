...
function TopMenu({ onTabChange }: { onTabChange?: (tab: string) => void }) {
  ...
  const allTabs = ['Calendário', 'Agendamentos', 'WhatsApp', 'Clientes', 'Análises', 'Cadastros', 'Configurações']
  const tabs = currentUser?.role === 'receptionist' 
    ? allTabs.filter(tab => tab !== 'Cadastros')
    : allTabs
