...
const templates: Record<TemplateKey, string> = {
  confirm:
    "Olá [Nome], confirmamos seu horário hoje às [Hora]. Qualquer dúvida, estamos à disposição!",
...
function generateMessage(
  ...
  data: { patient?: string; title?: string; start?: string },
...
export function WhatsAppView() {
  ...
  const clientMap = useMemo(() => {
    ...
    clients.forEach((c) =>
      map.set(c.id, { name: c.name, phone: c.mobile || c.phone, birthday: c.birthDate })
    );
    return map;
  }, [clients]);
  ...
  const rescueAppointments = useMemo(() => {
    ...
    return appointments
      .filter(
        (a) =>
          (a.status === "cancelled" || a.status === "cancelado") && !futureByClient.get(a.clientId)
      )
      .sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
  }, [appointments]);
  ...
  useEffect(() => {
    ...
    const professionalName = selected.professionalId
      ? profMap.get(selected.professionalId)
      : undefined;
  ...
  const professionalName = selected
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[380px,1fr] gap-6">
      ...
      <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-xl rounded-3xl p-4 flex flex-col gap-4">
        ...
        <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-900">
                    {client?.name ?? apt.patient ?? "Paciente"}
                  </p>
          ...
          {dataset.length === 0 && (
            <p className="text-sm text-gray-600">Nenhum item neste filtro.</p>
          )}
        </div>
      </div>
        ...
        {selected ? (
          <>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {selectedClient?.name ?? selected.patient ?? "Paciente"}
                </p>
