...
export function TenantManagementView() {
  ...
  const loadOrganizations = async () => {
    try {
      const { data, error } = await supabase
        .from("organizations")
        .select(
          "id, name, status, subscription_plan_id, subscription_renewal_date, platform_fee_override_percent, included_users_count, created_at"
        )
  ...
  const handleChangeStatus = async (
    clinicId: string,
    newStatus: "active" | "suspended" | "pending_setup"
  ) => {
    setProcessing(clinicId);
    try {
  ...
  const getStatusBadge = (status: string | null) => {
    ...
    return (
      <span
        className={`text-xs font-semibold px-2 py-1 rounded-lg flex items-center gap-1 ${statusInfo.color}`}
      >
                    ...
                    <tr key={org.id} className="border-b border-gray-100 hover:bg-white/50">
                      ...
                      <td className="py-4 px-4">
                        <p className="text-sm text-gray-900">
                          {formatDate(org.subscription_renewal_date)}
                        </p>
                      </td>
                      <td className="py-4 px-4">
                        <p className="text-sm text-gray-900">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleChangeStatus(
                                org.id,
                                e.target.value as "active" | "suspended" | "pending_setup"
                              )
                            }
                            disabled={processing === org.id}
