...
export function SchedulerView() {
  ...
  const [timeOffModalData, setTimeOffModalData] = useState<{ profId: string; date: Date } | null>(
    null
  );
  ...
  const filteredAppointments = useMemo(
    () => {
      ...
      console.log("🔍 SchedulerView - Filtrando agendamentos:", {
        ...
        professionalIdsNosAgendamentos: appointments.map((a) => ({
          id: a.id,
  ...
  const handleAppointmentDrop = useCallback(
    async (appointment: Appointment, newProfId: string, newTime: string) => {
      try {
          ...
          {visibleProfessionals.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-gray-600 text-sm">
                Nenhum profissional cadastrado. Cadastre profissionais na aba "Cadastros".
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="sticky top-0 z-10 bg-white/70 backdrop-blur-xl border-b border-white/50">
                ...
                >
                  <div className="h-16 px-4 flex items-center text-xs font-semibold text-gray-500">
                    Hora
                  </div>
                  {visibleProfessionals.map((p) => (
                    <div
                      key={p.id}
                      className="h-16 px-4 flex items-center gap-3 border-l border-gray-200/50"
                    >
                      {p.avatar ? (
                        <img
                          src={p.avatar}
                ...
                >
                    ...
                    visibleProfessionals.map((p, colIdx) => {
                      ...
                      if (workSchedule) {
                        ...
                        if (!workSchedule.days.includes(dayOfWeek)) {
                          ...
                          const [workStartHour, workStartMin] = workSchedule.start
                            .split(":")
                            .map(Number);
                          ...
                          if (slotMinutes < workStartMinutes || slotMinutes >= workEndMinutes) {
                            ...
                            const [breakStartHour, breakStartMin] = workSchedule.breakStart
                              .split(":")
                              .map(Number);
                            const [breakEndHour, breakEndMin] = workSchedule.breakEnd
                              .split(":")
                              .map(Number);
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            title={
                              !isAvailable
                                ? "Horário fora do turno do profissional"
                  ...
                  {filteredAppointments.map((ev) => {
                    ...
                    const evEnd = new Date(
                      ev.end || new Date(evStart.getTime() + (ev.durationMinutes || 30) * 60000)
                    );
                    const hasConflict = filteredAppointments.some((other) => {
                      if (other.id === ev.id || other.professionalId !== ev.professionalId)
                        return false;
                      const otherStart = new Date(other.start);
                      const otherEnd = new Date(
                        other.end ||
                          new Date(otherStart.getTime() + (other.durationMinutes || 30) * 60000)
                      );
                      return (
                    ...
                    let colIndex = -1;
                    if (
                      ev.professionalId === "all" ||
                      !ev.professionalId ||
                      ev.professionalId === null ||
                      ev.professionalId === undefined
                    ) {
                      ...
                      if (colIndex === -1) {
                        const allProfIndex = professionals.findIndex(
                          (p) => p.id === ev.professionalId
                        );
                        if (allProfIndex !== -1) {
                          ...
                          if (filters[prof.id] === false) {
                            ...
                            console.warn(
                              "⚠️ SchedulerView - Profissional está desmarcado nos filtros:",
                              {
                                appointmentId: ev.id,
                            ...
                            colIndex = visibleProfessionals.length > 0 ? 0 : -1;
                            console.warn(
                              "⚠️ SchedulerView - Profissional não está visível, renderizando na primeira coluna:",
                              {
                                appointmentId: ev.id,
                                professionalId: ev.professionalId,
                                professionalName: prof.name,
                          ...
                          console.error(
                            "❌ SchedulerView - professionalId não encontrado em nenhum profissional, NÃO renderizando:",
                            {
                              appointmentId: ev.id,
                              professionalId: ev.professionalId,
                              visibleProfessionalsCount: visibleProfessionals.length,
                    ...
                    if (colIndex === -1) {
                      console.error(
                        "❌ SchedulerView - Agendamento não renderizado (nenhum profissional visível):",
                        {
                          appointmentId: ev.id,
                          professionalId: ev.professionalId,
                          visibleProfessionalsCount: visibleProfessionals.length,
                    ...
                    if (Math.random() < 0.1) {
                      // Log apenas 10% dos agendamentos
                    ...
                    return (
                      <EventCard
                        ...
                        onDrop={(newProfId, newTime) =>
                          handleAppointmentDrop(ev, newProfId, newTime)
                        }
                  ...
                  {blocks
                    ...
                    .map((b) => {
                      ...
                      const colIndex = visibleProfessionals.findIndex(
                        (p) => p.id === b.professionalId
                      );
...
function mapStatusToPortuguese(
  status?: string
): "agendado" | "confirmado" | "atendimento" | "concluído" | "cancelado" | "falta" {
...
function AppointmentModal({
  ...
  const defaultService =
    services.length > 0 ? services[0] : { id: "", name: "Nenhum serviço", price: 0, duration: 30 };
  const [isLoading, setIsLoading] = useState(false);
  const { register, watch, setValue, handleSubmit, reset } = useForm<AppointmentForm>({
    defaultValues: {
      clientQuery: "",
      clientId: "",
      patient: "",
  ...
  useEffect(() => {
    ...
    if (initialData.event) {
      ...
      const eventDate = initialData.event.start
        ? new Date(initialData.event.start)
        : initialData.date;
  ...
  const onSubmit = async (data: AppointmentForm) => {
    ...
    try {
      ...
      if (data.isRecurring && data.recurringCount && data.recurringCount > 1) {
        ...
        if (successCount > 0) {
          toast.success(
            `${successCount} agendamento(s) recorrente(s) criado(s) com sucesso!${errorCount > 0 ? ` (${errorCount} falharam)` : ""}`
          );
        } else {
          ...
          <div className="bg-white/50 rounded-xl border border-white/60 shadow-sm p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-900">
                Paciente solicitou este profissional?
              </label>
              ...
              <>
                <div className="md:col-span-2 space-y-2">
                  <label className="flex items-center gap-2 text-sm font-semibold text-gray-900">
                    <input type="checkbox" {...register("isRecurring")} className="h-4 w-4" />
                    Agendamento Recorrente
                  </label>
                </div>
                {watch("isRecurring") && (
...
function TimeOffModal({
  ...
  const { register, handleSubmit, reset } = useForm<TimeOffForm>({
    defaultValues: {
      professionalId: initialData?.profId ?? professionals[0]?.id,
      startDate:
...
function EventCard({
  ...
  // ✅ MOVER HOOKS PARA ANTES DOS EARLY RETURNS
  ...
  return (
    <div
      ...
      data-professional-id={
        type === "appointment" ? (style as any)?.professionalId || "" : undefined
      }
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {status === "confirmado" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-emerald-600 bg-emerald-100"
            aria-label="Confirmado"
          />
        )}
        {status === "agendado" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-blue-500 bg-blue-50"
            aria-label="Agendado"
          />
        )}
        {status === "atendimento" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-purple-500 bg-purple-50"
            aria-label="Em Atendimento"
          />
        )}
        {status === "concluído" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-green-600 bg-green-100"
            aria-label="Concluído"
          />
        )}
        {status === "cancelado" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-red-500 bg-red-50"
            aria-label="Cancelado"
          />
        )}
        {status === "falta" && (
          <span
            className="h-4 w-4 rounded-full border-2 border-orange-500 bg-orange-50"
            aria-label="Falta"
          />
        )}
      </div>
